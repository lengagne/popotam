...
class Problem1 : public AbstractCSP
{
    public:
        Problem1(unsigned int n,double X, double Y):AbstractCSP()
        {
			nb_dof_ = n;
            pb_name_ = "2D_Problem1_Ndof"+std::to_string(nb_dof_)+"_X"+std::to_string(X)+"_Y"+std::to_string(Y);
            Xt = X;
            Yt = Y;
            get_criteria_ = true;
